...
const AFFIRMATIVE_RE =
  /^\s*(yes|yeah|yep|sure|please|ok(ay)?|go ahead|do it|absolutely|of course)\b/i;
...
export default function StagePage() {
  ...
  const speakChunks = useCallback(
    async (chunkPromises: Promise<ArrayBuffer>[], fullText: string) => {
  ...
  const sendMessage = useCallback(
    async (userText: string) => {
      ...
      const isConfirmingOffer =
        offeredIndex !== null && AFFIRMATIVE_RE.test(userText);
          ...
          try {
            ...
            if (delta) {
              if (firstChunk) {
                ...
                firstChunk = false;
                videoSelectPromise =
                  isConfirmingOffer && offeredIndex !== null
      ...
      setPhase("speaking");
      setReply(cleanReply);
      const rest = firstTtsPromise
        ? cleanReply.slice(firstSentence.length).trim()
        : "";
      if (firstTtsPromise) {
  ...
  const finalizeRecording = useCallback(
    (finalTranscript: string) => {
      ...
      if (
        /^(hi+|hello|hey|howdy|greetings|hi there|hello there|hey there|good morning|good afternoon|good evening)[\s!.]*$/i.test(
      ...
      const normalized = finalTranscript
        ...
        .replace(
          /\b(zaar\s*br[uü]?c?k?e?n?|zaar\s*bguken|zaar\s*brook|saar\s*br[uü]?c?k?e?n?|sar\s*brook|zarbrook|saarbrucken|saarbrücken)\b/gi,
          "Saarbrücken",
        )

...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: () => void; label: string }) {
  return (
    ...
    >
      <div className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${checked ? "bg-indigo-500" : "bg-slate-700"}`}>
...
function NavItem({
  id, label, icon, active, onClick, done,
}: {
  id: NavSection; label: string; icon: React.ReactNode;
  active: boolean; onClick: () => void; done: boolean;
}) {
  return (
    <button
      onClick={onClick}
    ...
    >
      <span className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors ${
        active ? "bg-indigo-500/20 text-indigo-400" : "bg-slate-800 text-slate-500 group-hover:bg-slate-700 group-hover:text-slate-300"
      }`}>
        {icon}
      </span>
...
function SectionScene({ scene, onChange }: { scene: Scene; onChange: (s: Scene) => void }) {
  return (
    <SectionCard title="Scene" subtitle="The core identity of this experience.">
      ...
      <Field label="Knowledge & system prompt" hint="Describe who the character is, what they know, and how they speak. Keep replies short — this is a voice conversation.">
        <Textarea
          rows={6}
      ...
      <Field label="Idle message" hint="Shown before the visitor says anything.">
        <Input
        ...
        <Field label="Display options">
          <div className="flex flex-col gap-3 justify-center h-full pt-1">
            <Toggle
              ...
              onChange={() => onChange({ ...scene, showBotText: !scene.showBotText })}
...
function SectionCharacter({ scene, onChange }: { scene: Scene; onChange: (s: Scene) => void }) {
  return (
...
function VideoUploadZone({ clipIndex, onUploaded }: { clipIndex: number; onUploaded: (url: string) => void }) {
  ...
  const upload = useCallback(async (file: File) => {
    ...
    try {
      ...
      setProgress(100);
      setTimeout(() => { onUploaded(data.url); setProgress(0); }, 400);
    } catch (e) {
      clearInterval(interval);
...
function SectionVideos({ scene, onChange }: { scene: Scene; onChange: (s: Scene) => void }) {
  ...
  const addVideo = useCallback(() => {
    ...
    onChange({ ...scene, videos: [...scene.videos, { index: next, url: "", label: `Clip ${next}` }] });
  }, [scene, onChange]);

  const removeVideo = useCallback((index: number) => {
    const filtered = scene.videos.filter(v => v.index !== index).map((v, i) => ({ ...v, index: i + 1 }));
    onChange({ ...scene, videos: filtered });
  }, [scene, onChange]);

  const updateVideo = useCallback((index: number, field: keyof VideoClip, value: string | number) => {
    onChange({ ...scene, videos: scene.videos.map(v => v.index === index ? { ...v, [field]: value } : v) });
  }, [scene, onChange]);
  ...
  const runTest = useCallback(async () => {
    ...
    try {
      ...
      setResults(prev => [{ reply: testReply, expected, got, ok: expected !== null ? got === expected : null, loading: false }, ...prev.slice(0, 9)]);
    } catch {
      setResults(prev => [{ reply: testReply, expected, got: null, ok: false, loading: false }, ...prev.slice(0, 9)]);
    }
    setTesting(false);
  ...
  return (
    <div className="flex flex-col gap-4">
      <SectionCard title="Video Clips" subtitle="Upload one video per emotion or topic. The AI picks the right one automatically.">
        <div className="flex flex-col gap-3">
          {scene.videos.map(v => (
            <div key={v.index} className="bg-slate-800/40 border border-slate-700/40 rounded-xl p-4 flex gap-4 items-start">
      ...
      <SectionCard title="Selector Tester" subtitle="Type a sample bot reply to check which clip the AI would pick.">
        <Textarea
          rows={3}
          value={testReply}
        ...
        {results.length > 0 && (
          <div className="flex flex-col gap-2">
            <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Recent results</p>
            {results.map((r, i) => (
              <div key={i} className="bg-slate-800/60 rounded-xl px-4 py-3 flex items-center gap-3 text-sm">
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                  r.ok === true ? "bg-emerald-500/20 text-emerald-400" :
                  r.ok === false ? "bg-red-500/20 text-red-400" :
                  "bg-slate-700 text-slate-400"
                }`}>{r.got ?? "?"}</span>
                <span className="text-slate-400 truncate flex-1 text-xs">{r.reply}</span>
                {r.ok === false && <span className="text-xs text-red-400 shrink-0">expected {r.expected}</span>}
                {r.ok === true && <span className="text-xs text-emerald-400 shrink-0">✓ correct</span>}
              </div>
            ))}
          </div>
        )}
...
function SectionShare({ scene }: { scene: Scene }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Summary */}
      <SectionCard title="Scene summary" subtitle="Overview of your current configuration.">
      ...
      <SectionCard title="Share" subtitle="Give this link to visitors.">
        ...
        <p className="text-xs text-slate-600">Deploy to Vercel first to make this URL live.</p>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Print and place near your installation for visitors to scan.</p>
          </div>
        </div>
      </SectionCard>

      {/* Embed */}
      <SectionCard title="Embed" subtitle="Drop into any webpage or kiosk browser.">
    ...
    <div className="flex h-screen bg-[#0c0e14] text-white overflow-hidden">
      ...
      <aside className="w-56 shrink-0 flex flex-col border-r border-slate-800/80 bg-[#0e1018]">
        ...
        <div className="px-4 py-5 border-b border-slate-800/80">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-sm font-semibold text-white leading-none">AgentStage</p>
        ...
        <div className="px-4 py-3 border-b border-slate-800/80">
          ...
          <p className="text-sm text-white font-medium truncate">{scene.name || "Untitled scene"}</p>
          {scene.slug && (
            <p className="text-xs text-indigo-400/70 truncate mt-0.5">/{scene.slug}</p>
          )}
        </div>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="max-w-2xl mx-auto px-8 py-8">
          {active === "scene"     && <SectionScene     scene={scene} onChange={setScene} />}
          {active === "character" && <SectionCharacter scene={scene} onChange={setScene} />}
          {active === "videos"    && <SectionVideos    scene={scene} onChange={setScene} />}
